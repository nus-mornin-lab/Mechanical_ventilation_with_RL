...
if __name__ == "__main__":
    ...
    eicu_data = 'data/data_rl_60min_eicu.csv'#'data/data_rl_with_dose.csv' #
    mimic_data = 'data/data_rl_60min_mimic.csv'#'data/mimic_data_rl_with_dose_11Dec.csv' 
    cut_mimic_data = 'data/data_rl_60min_mimic_cut.csv'
    ...
    if setting.VALIDATION == True:
        ...
        val_paras['batch_size'] = 2**np.arange(0,3)*64  # 64, 128, 256
        val_paras['tau'] = [0.01,0.02]
        val_paras['lr'] = [0.001,0.002]
        ...
        for ind in val_res.index:
            ...
            v_cwpdis, ess = cal_cpwdis(val_dt, parameters)
    ...
    # save model
    mod_dir = 'model/' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '_' + setting.REWARD_FUN  +'_' + str(SEED) + '_' + setting.MODEL + '_' + val_str + '/'
    ...
    evaluation_new.run_eval(data_,loss_,datatype, setting.SEED, 'train', parameters, val_str, val_res)
    ...
    evaluation_new.run_eval(test_data,False,datatype, setting.SEED, 'innertest', parameters, val_str, pd.DataFrame())
    
    # evaluate on test_out set
    outer_test_data = pd.read_csv(eicu_data)
    
    outer_test_data['step_id'] = outer_test_data['step_id'].astype(int)
    ...
    evaluation_new.run_eval(outer_test_data,False,datatype_, setting.SEED, 'outtertest', parameters, val_str, pd.DataFrame())
