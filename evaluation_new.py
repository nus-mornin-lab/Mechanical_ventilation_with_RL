...
def run_eval(data, loss=None):
    ...
    def diff_vs_motality(data, outcome_col = 'hosp_mort'):
    ...
    # vs_motality
        ...
        used_data = data[[v + '_diff' for v in action_types] + [outcome_col]]
        for i in range(2000):
            if i%10 == 0:
            ...
            for v in action_types:
                diff_col = v + '_diff'
                aa = used_data.loc[df_index].groupby(diff_col).apply(lambda dt: dt[outcome_col].mean())
                for k in x_s[v]:
                    res[v][k].append(aa[k])
        ...
        for v in action_types:
            ...
            for k in x_s_b[v]:
                res_b[v][k] = []
        used_data = data[[v + '_diff_mean_level' for v in action_types] + [v + '_conc_rate_level' for v in action_types] + ['patientunitstayid',outcome_col]].drop_duplicates(subset = ['patientunitstayid'])
        for i in range(2000):
            if i%10 == 0:
            ...
            for v in action_types:
                ...
                aa = used_data.loc[df_index].groupby(diff_mean_col).apply(lambda dt: dt[outcome_col].mean())
                bb = used_data.loc[df_index].groupby(conc_rate_col).apply(lambda dt: dt[outcome_col].mean())
                for k in x_s_a[v]:
                    if k in aa.index:
        ...
        # plot 
        def plot_vs_motality(x_s, res, x_name, title_name,color_):
            res_025 = {}
            res_500 = {}
            res_975 = {}
            for v in action_types:
        ...
        plot_vs_motality(x_s, res, 'Model action - Phys action', 'diff4h_vs_motality', 'red')
        plot_vs_motality(x_s_a, res_a, 'Model action - Phys action', 'diffPatientlevel_vs_motality','brown')
        plot_vs_motality(x_s_b, res_b, 'Concordant rate', 'conc_vs_motality','green')

    def q_vs_motality(data, outcome_col = 'hosp_mort'):
        # Q_vs_motality    
        ...
        bb = data.groupby(phys_Q + '_discre').apply(lambda dt: [dt[outcome_col].mean(),dt[outcome_col].sem()])
        res_025_Q = []
        res_500_Q = []
        res_975_Q = []
        for k in Q_s:
    ...
    def quantitive_eval(data,res_dt, outcome_col = 'hosp_mort'):
        ...
        def find_nearest_Q(Q_mean):
            ind = np.argmin([abs(Q_mean - i) for i in res_dt.index])
            Q_res = res_dt.index[ind]
            return Q_res
        
        for mod in ['ai','phys','Random','One-size-fit-all']:
            q_dr_dt.loc[mod,'dr'] = res_dt.loc[find_nearest_Q(q_dr_dt.loc[mod,'Q']),'dr']
            
        q_dr_dt.to_csv(res_dir + 'qmean_and_deathrate.csv', encoding = 'gb18030')
