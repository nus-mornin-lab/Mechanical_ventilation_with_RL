...
SEED = 603                                 # 可调  主要影响参数初始化和训练时选择数据的顺序
ITERATION_ROUND = 40                     # 可调  大约遍历数据集多少次
ACTION_SPACE = 18# 27
BATCH_SIZE = 256
...
CUT_TIME = '48h'   # 48h/72h/14d           可调  截断时间，episode长度
TRAIN_SET = 'eicu'   # mimic/eicu         可调  训练数据集，在mimic/eicu上训练 （在另一个数据集上测试）
STEP_LENGTH = '240min'   # 240min/60min    可调  每个time_step的长度
CRITICAL_STATE = False   # True/False       可调  是否仅使用关键state

REWARD_FUN = 'reward_short_long_usd_H'     # 可调  在本py中定义新的reward函数
