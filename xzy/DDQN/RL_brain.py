...
class FQI:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            self.done = tf.placeholder(tf.float32, [None, self.n_actions], name='done')
            self.q_next = tf.multiply(self.q_eval, self.done)            
    ...
    def learn(self, i):
        ...
        # sample_index = np.random.choice(self.memory_size, size=self.batch_size)
        sample_index = np.arange(self.memory_size)
        batch_memory = self.memory[sample_index, :]

        done_vec = np.tile((1-batch_memory[:, self.n_features + 2]).reshape(-1, 1), self.n_actions)
        
        # q_next_raw = self.sess.run(self.q_eval, feed_dict={self.s: batch_memory[:, -self.n_features:]}) # next observation
        q_next = self.sess.run(self.q_eval, feed_dict={self.s: batch_memory[:, -self.n_features:], self.done: done_vec}) # next observation
        ...
        batch_index = np.arange(self.memory_size, dtype=np.int32)
        eval_act_index = batch_memory[:, self.n_features].astype(int)
        reward = batch_memory[:, self.n_features + 1]
